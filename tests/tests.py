...
from staticfiles import finders, settings, storage
...
if not hasattr(TestCase, 'assertRaisesRegexp'):
    ...
    class TestCase(DTestCase, U2TestCase):
        pass
...
class StaticFilesTestCase(TestCase):
    ...
    """
    def setUp(self):
        self.old_staticfiles_url = settings.URL
        self.old_staticfiles_root = settings.ROOT
        self.old_staticfiles_dirs = settings.DIRS
        self.old_staticfiles_finders = settings.FINDERS
        self.old_media_root = django_settings.MEDIA_ROOT
        self.old_media_url = django_settings.MEDIA_URL
        self.old_admin_media_prefix = django_settings.ADMIN_MEDIA_PREFIX
        self.old_debug = django_settings.DEBUG
        self.old_installed_apps = django_settings.INSTALLED_APPS

        # We have to load these apps to test staticfiles.
        load_app('tests.apps.test')
        load_app('tests.apps.no_label')
        site_media = os.path.join(django_settings.TEST_ROOT, 'project', 'site_media')
        django_settings.DEBUG = True
        django_settings.MEDIA_ROOT =  os.path.join(site_media, 'media')
        django_settings.MEDIA_URL = '/media/'
        django_settings.STATICFILES_ROOT = os.path.join(site_media, 'static')
        django_settings.STATICFILES_URL = '/static/'
        django_settings.ADMIN_MEDIA_PREFIX = '/static/admin/'
        django_settings.STATICFILES_DIRS = (
            os.path.join(django_settings.TEST_ROOT, 'project', 'static'),
        )
        django_settings.STATICFILES_FINDERS = (
    ...
    def tearDown(self):
        django_settings.DEBUG = self.old_debug
        django_settings.MEDIA_ROOT = self.old_media_root
        django_settings.MEDIA_URL = self.old_media_url
        django_settings.ADMIN_MEDIA_PREFIX = self.old_admin_media_prefix
        django_settings.STATICFILES_ROOT = self.old_staticfiles_root
        django_settings.STATICFILES_URL = self.old_staticfiles_url
        django_settings.STATICFILES_DIRS = self.old_staticfiles_dirs
        django_settings.STATICFILES_FINDERS = self.old_staticfiles_finders
...
class BuildStaticTestCase(StaticFilesTestCase):
    ...
    This relies on the asserts defined in StaticFilesTestCase, but
    ...
    def setUp(self):
        ...
        django_settings.STATICFILES_ROOT = settings.ROOT = tempfile.mkdtemp()
        self.run_collectstatic()

    def tearDown(self):
        shutil.rmtree(settings.ROOT)
        django_settings.STATICFILES_ROOT = settings.ROOT = self.old_root
...
if sys.platform != 'win32':
    class TestBuildStaticLinks(BuildStaticTestCase, TestDefaults):
        ...
        def test_links_created(self):
            ...
            """
            self.failUnless(os.path.islink(os.path.join(settings.ROOT, 'test.txt')))
...
class TestServeDisabled(TestServeStatic):
    """
    Test serving media from django.contrib.admin when DEBUG=False.
    """
    def setUp(self):
...
class TestAppDirectoriesFinder(StaticFilesTestCase, FinderTestCase):
    ...
    def setUp(self):
        ...
        test_file_path = os.path.join(django_settings.TEST_ROOT, 'apps/test/static/test/file1.txt')
...
class TemplateTagTest(TestCase):
    def test_get_staticfiles_prefix(self):
        ...
        ).render(Context()), django_settings.STATICFILES_URL)
